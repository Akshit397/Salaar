...
const usersListingStages = [
  ...
  { $unwind: { path: "$country", preserveNullAndEmptyArrays: true } },
  {
    $project: {
      ...
      country: 1,
      state: 1, city: 1, zipCode: 1,
      organisationName: 1, registeredYear: 1,
      roleInOrganisation: 1, dob: 1, userRoleName: 1,
    },
  },
];
...
class UserManagementController extends Controller {
  ...
  /********************************************************
       Purpose: Change status of Single And Multiple Users Details In Admin
       Method: Post
       Authorisation: true
       Parameter:
       {
       ...
       Return: JSON String
       ********************************************************/
  async changeStatusOfUsersByAdmin() {
    try {
      let model = this.req.model ? this.req.model : Users;
      let msg = 'User details not updated.';
      const updatedUser = await model.updateMany({ _id: { $in: this.req.body.userIds } }, { $set: { status: this.req.body.status } });
      if (updatedUser) {
        msg = updatedUser.modifiedCount ? updatedUser.modifiedCount + ' User details updated.' : updatedUser.n == 0 ? "Details not found" : msg;
      }
