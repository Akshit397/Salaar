...
const kycSellersListingStages = [
    ...
    { $unwind: { "path": "$country", "preserveNullAndEmptyArrays": true } },
    {
        $project: {
            ...
            kycDetails: 1, country: 1, bankDetails: 1, etdDetails: 1, fssaiDetails: 1, iecDetails: 1, signatureDetails: 1
        }
    }
]
...
class SellerManagementController extends Controller {
    ...
    ********************************************************/
    ...
    /********************************************************
         Purpose: Change status of Single And Multiple Sellers Details In Admin
         Method: Post
         Authorisation: true
         Parameter:
         {
         ...
         Return: JSON String
         ********************************************************/
    async changeStatusOfSellersByAdmin() {
        try {
            let model = this.req.model ? this.req.model : Sellers;
            let msg = 'Seller details not updated.';
            const updatedSeller = await model.updateMany({ _id: { $in: this.req.body.sellerIds } }, { $set: { status: this.req.body.status } });
            if (updatedSeller) {
                msg = updatedSeller.modifiedCount ? updatedSeller.modifiedCount + ' Seller details updated.' : updatedSeller.n == 0 ? "Details not found" : msg;
            }
