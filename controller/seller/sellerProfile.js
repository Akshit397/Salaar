  ...
  ********************************************************/
   ...
   ********************************************************/
        ...
        try {
            ...
            if (currentSellerId) {
                const seller = await Sellers.findOne({ _id: currentSellerId, isDeleted: false, status: true, }, { password: 0, transactionPassword: 0, _v: 0 })
                    .populate('countryId', { name: 1, iso: 1, nickname: 1, countryId: 1 })
                    .populate('mailingAddress.countryId', { name: 1, iso: 1, nickname: 1, countryId: 1 })
                    .populate('segmentId', { name: 1 })
                    .populate('subSegmentId', { name: 1 });
                if (_.isEmpty(seller)) {
                    return this.res.send({ status: 0, message: "Seller not found" });
                }
    ...
    Parameter:
    {
        ...
        "subSegmentId":"640c67c21fec4c7a6785b359",
    ...
    ********************************************************/
    async editSellerProfile() {
        try {
            ...
            if (data.mailingAddress && data.mailingAddress.countryId) {
                const validateCountry = await Country.findOne({ _id: data.mailingAddress.countryId });
            ...
            if (data.segmentId) {
                const validateSegment = await BusinessSegments.findOne({ _id: data.segmentId });
                if (_.isEmpty(validateSegment)) {
                    return this.res.send({ status: 0, message: "Business segment details not found" })
                }
            }
            if (data.subSegmentId) {
                const validateSubSegment = await BusinessSubSegments.findOne({ _id: data.subSegmentId });
